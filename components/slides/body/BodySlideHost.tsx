...
import { AnimatedImage, DraggableImage } from '../AnimatedImage';
...
export function BodySlideHost({
  ...
  const showImage = !skipImageSlot && slide.image.on && slide.image.item;
  ...
  return (
    ...
    >
      ...
      {/* 자유 이미지 슬롯 (드래그) */}
      {showImage && slide.image.item && (
        editable && onImageUpdate ? (
          <div style={{ position: 'absolute', inset: 0, zIndex: 3 }}>
            <DraggableImage image={slide.image.item} onUpdate={onImageUpdate} containerScale={containerScale} />
          </div>
        ) : (
          <div style={{ position: 'absolute', inset: 0, zIndex: 3 }}>
            <AnimatedImage image={slide.image.item} />
          </div>
        )
      )}
    </div>
